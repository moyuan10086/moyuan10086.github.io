(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory();else if(!root.MoyuanMusicSource)root.MoyuanMusicSource=factory();})(typeof globalThis!=='undefined'?globalThis:this,function(){
 'use strict';
 const cache=new Map(),localFiles=new Map();
 ...
 function matches(track,candidate){
  return normalize(track.name)===normalize(candidate.song)&&normalize(track.artist)===normalize(candidate.singer)&&durationMatches(track,seconds(candidate.time));
 }
 ...
 async function resolveCloud(track,options={}){
  ...
  const params=new URLSearchParams({msg:track.name+' '+track.artist});
  ...
  const result={url:url.href,provider:'kw',matchedId:resolved.rid,originalSource:track.source,label:'酷我音乐'};
 ...
 async function resolve(track,options={}){
  ...
  try{return await resolveCloud(track,options);}
  catch(error){
   if(error.name==='AbortError'||options.signal?.aborted)throw error;
   if(track.file)return {url:assetUrl(track.file),provider:'local',label:'本站备份'};
   throw error;
  }
 }
